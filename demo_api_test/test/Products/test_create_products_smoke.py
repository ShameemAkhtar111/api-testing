...
def test_create_1_simple_product():
    ...
    assert payload['name'] == db_product[0]['post_title'], f"Create product title in does not match with title in api " \
                                                           f"response. DB: {db_product['post_title']}, API: {payload['name']} "
